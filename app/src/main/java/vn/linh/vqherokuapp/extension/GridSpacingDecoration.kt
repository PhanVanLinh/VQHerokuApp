...
open class GridSpacingDecoration(private val top: Int, private val right: Int,
    private val bottom: Int, private val left: Int,
    private val middle: Int) : RecyclerView.ItemDecoration() {
    ...
    override fun getItemOffsets(outRect: Rect, view: View,
        ...
        outRect.right = horizontalSpaces!![(column + spanSize - 1) * 2 + 1]
        if (!isFirstRow(row)) {
            outRect.top = middle
        }
        if (isFirstRow(row)) {
    ...
    private fun initHorizontalSpaces(spanCount: Int, recyclerWidth: Int, start: Int, end: Int,
        middle: Int): IntArray {
        val horizontalSpaces = IntArray(spanCount * 2)
        val itemWidthFull = recyclerWidth / spanCount
        val itemWidthAfterAddSpacing = (recyclerWidth - start - end - middle * (spanCount - 1)) / spanCount
